...
| 카테고리 | 검색 대상 |
|------|------|
| **OTT** | 넷플릭스·티빙·웨이브·디즈니플러스 × 인기/신작/추천 × 드라마/영화 |
| **웹툰·만화책** | 웹툰·만화책 × 인기/신작/추천 |
| **보드게임** | 보드게임 × 인기/신작/추천 |

총 **33개 검색 쿼리**로 수집한 뒤, 아래 2단계 필터를 거쳐 **조회수 + 검색 정확도(관련도)** 기준으로 순위를 매깁니다.

1. **카테고리 관련성 검증** — 제목에 실제로 해당 카테고리 키워드(플랫폼명·영화·드라마·웹툰·만화책·보드게임 등)가 포함된 영상만 채택. YouTube 검색이 무관한 영상으로 결과를 채우는 문제를 여기서 걸러냄.
2. **최소 조회수 필터** — 조회수 3,000회 미만 영상은 후보에서 완전히 제외.
