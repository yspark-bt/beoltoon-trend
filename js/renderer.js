...
function renderTop15(top15, analysis) {
  ...
  top15.forEach((t, idx) => {
    ...
    const tagsHtml = (a.tags || []).map(tg => {
      let cls = 'dtag';
      if (tg.includes('정보 나열형')) cls += ' info-only';
      else if (tg.includes('감성 무드형')) cls += ' experience';
      else if (tg.includes('화제작형')) cls += ' buzz';
    ...
    const storeGuideHtml = a.storeGuide ? `
      <div class="ablock store-guide">
        <div class="ablock-label store">🏪 벌툰 매장 촬영법 — ${esc(a.storeGuide.formatName)}</div>
        <p class="ablock-text">${esc(a.storeGuide.guide)}</p>
      </div>` : '';

    const bridgeHtml = a.bridgeLine ? `
      <div class="ablock bridge-line">
        <div class="ablock-label bridge">🔗 벌툰 연결 멘트 — 이 한 줄이 방문을 결정합니다</div>
        <p class="ablock-text">${esc(a.bridgeLine)}</p>
      </div>` : '';
    ...
    panel.innerHTML = `
      <div class="detail-grid">
        ...
        <div class="detail-right">
          <div class="ablock">
            <div class="ablock-label cyan">📊 왜 반응이 좋을까</div>
            ${statsHtml}
          ...
          ${storeGuideHtml}
          <div class="ablock">
            <div class="ablock-label navy">✅ 이렇게 찍으면 됩니다</div>
...
function renderActions(actions) {
  document.getElementById('xcardsEl').innerHTML = (actions || []).map((a, i) => {
    const storeGuideBlock = a.storeGuide ? `
      <div class="action-store-guide">
        <div class="asg-label">🏪 벌툰 매장 촬영법 — ${esc(a.storeGuide.formatName)}</div>
        <p>${esc(a.storeGuide.guide)}</p>
      </div>` : '';
    const bridgeBlock = a.bridgeLine ? `
      <div class="action-bridge">
        <div class="ab-label">🔗 벌툰 연결 멘트 (필수 삽입)</div>
        <p>${esc(a.bridgeLine)}</p>
      </div>` : '';
    const badges = [];
    if (a.isBuzz)       badges.push(`<span class="action-rep-badge buzz">🔥 화제작형</span>`);
    if (a.isExperience) badges.push(`<span class="action-rep-badge exp">✨ 감성 무드형</span>`);
    if (a.isReplicable) badges.push(`<span class="action-rep-badge">🏪 매장 재현 가능</span>`);
