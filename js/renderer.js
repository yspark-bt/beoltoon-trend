/**
 * renderer.js  v2.0
 * fika.insight 스타일 아코디언 TOP 15
 * — 행 클릭 → 바로 아래 패널 슬라이드 오픈
 * — 왼쪽: YouTube 영상 임베드 / 오른쪽: 분석+액션 2단
 */
...
function renderTop15(top15, analysis) {
  ...
  hdr.innerHTML = `<span>#</span><span>변동</span><span>트렌드</span><span class="th-score">점수</span><span></span>`;
  el.appendChild(hdr);
  ...
  top15.forEach((t, idx) => {
    ...
    const ytBlock = a.videoId
      ...
      : `<a class="yt-search-card" href="https://www.youtube.com/results?search_query=${encodeURIComponent(t.label+' 쇼츠')}" target="_blank" rel="noopener">
           ...
           <div class="yt-info"><div class="yt-title">${esc(t.label)} 관련 영상 보기</div></div>
         </a>`;

    const tagsHtml = (a.tags || []).map(tg => `<span class="dtag">${esc(tg)}</span>`).join('');
    ...
    panel.innerHTML = `
      <div class="detail-grid">
        <div class="detail-left">
          <div class="detail-sec-label">관련 영상</div>
          ${ytBlock}
        </div>
        <div class="detail-right">
          <div class="ablock">
            <div class="ablock-label cyan">📊 트렌드 분석</div>
            ${statsHtml}
            <p class="ablock-text">${a.content || ''}</p>
            <div class="dtags">${tagsHtml}</div>
          </div>
          <div class="ablock">
            <div class="ablock-label navy">✅ 액션 아이템</div>
...
function renderPicks(picks) {
  document.getElementById('pgridEl').innerHTML = (picks || []).map(p => `
    <div class="pick-card">
      <div class="pick-label">⚡ 선점 기회 · 점수 ${p.score} · 영상 ${p.videoCount}개</div>
      ...
      }" target="_blank" rel="noopener" class="pick-link">▶ 영상 확인 →</a>
    </div>`).join('');
}
    ...
    <div class="action-card">
      <div class="action-hdr">
        ...
        <div class="action-meta">
          ...
          <span class="action-views">👁 ${fmt(a.viewCount)}</span>
        </div>
      </div>
      <div class="action-body">
        <div class="action-ref">📺 참고: <a href="${
          a.exampleVideoId
