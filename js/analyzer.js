/**
 * analyzer.js  v6.0 — 단순화
 *
 * 순위 정렬 기준: 조회수 + 정확도(검색 관련도)만 사용
 * 복잡한 경험형/화제작형/정보나열형 분류, 브릿지 멘트 로직은 전부 제거
 *
 * 점수 공식 (영상 1개 단위):
 *   조회수 × 0.65
...
export function analyzeAndRank(videos) {
  ...
  const ranked = top15.map((v, i) => {
    ...
    return {
      ...
      category: classifyCategory(v.keyword),
  ...
  const picks = valid.slice(15, 22).map(v => ({
    ...
    videoId: v.videoId,
    videoTitle: v.title,
    desc: `[${classifyCategory(v.keyword)}] 조회수 <strong>${fmt(v.viewCount)}회</strong>, 좋아요 <strong>${fmt(v.likeCount)}개</strong>를 기록한 콘텐츠입니다.`,
  }));
