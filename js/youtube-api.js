 ...
 * youtube-api.js  v9.0
 *
 * 목적: 만화카페 '벌툰' 방문객이 "가고 싶다"는 마음이 들게 하는
 *       YouTube Shorts를 발굴 + 가맹점이 그대로 따라 촬영할 수 있는 포맷 추출
 *
 * ★ v9 핵심 변경: 방문 욕구의 실제 심리 사슬 반영
 *   "이 만화책 재밌어 보이네 → 벌툰 가서 봐야지" 처럼
 *   화제작 정보(무엇이 뜨는지)와 벌툰 브릿지(여기서 볼 수 있다)가
 *   합쳐져야 방문 욕구가 생긴다는 점을 반영해 3가지로 재분류:
 *     [1] isBuzz       — 화제작형 (구체적 작품·이슈가 화제성 있게 언급)
 *     [2] isExperience — 경험형 (실제 공간·관계·감정이 담긴 콘텐츠)
 *     [3] isInfoOnly   — 순수 정보나열형 (위 둘 다 없는 기계적 리스트만 해당,
 *                        범위를 좁혀 화제작/경험 신호가 있으면 여기서 제외)
 *   → analyzer.js에서 isBuzz·isExperience 콘텐츠 모두 우선 노출하고,
 *     매장 가이드·액션 아이템에 "벌툰에서 볼 수 있어요" 브릿지 멘트를 필수로 삽입
 *
 * 벌툰 방문 목적 5가지 (실제 방문 동기 기준):
 *   1. 놀러오기 — 친구들과 아지트, 시간 보내기
 *   2. 데이트   — 커플 데이트 코스, 프라이빗 공간
 *   3. OTT 시청 — 넷플릭스·티빙·웨이브·디즈니플러스 드라마·영화·예능 감상
 ...
 *       웹툰·만화책 / OTT(넷플릭스·티빙·웨이브·디즈니플러스)·예능 / 보드게임
 *
 ...
 *
 *   STEP 1B. 관심사·포맷별 검색으로 보완 수집           → ~4,600 units
 *            웹툰·만화책(7) / OTT 4플랫폼×3장르×2타입(24) /
 *            보드게임(3) / 카페투어(4) / 혼놀(2) / 친구모임(2) /
 *            데이트(2) / 핫플(2)  = 총 46개 검색 쿼리
 *
 ...
 *
 *   STEP 4.  역추출 키워드로 추가 검색                 → ~1,200 units
 *            상위 12개 키워드로 최근 7일 Shorts 수집 (쿼터 절약 위해 20→12 축소)
 *
 *   STEP 5.  전체 통계 최종 보강                       → ~20 units
 *
 * 총 쿼터: ~5,839 units / 1일 약 1회 분석 가능
 *   ※ OTT 플랫폼×장르×타입 전체 조합(24개 쿼리) 추가로 쿼터 소모가 큽니다.
 *     분석 횟수보다 커버리지가 중요한 경우에 적합합니다.
...
═══════════════════════════════════════════════════ */
// [A] OTT 플랫폼 × 장르 × 트렌드타입 전체 조합
//     4개 플랫폼 × (영화·드라마·예능) × (추천·인기) = 24개 쿼리
const OTT_PLATFORMS = ['넷플릭스', '티빙', '웨이브', '디즈니플러스'];
const OTT_GENRES    = ['영화', '드라마', '예능'];
const OTT_TRENDS    = ['추천', '인기'];
const OTT_QUERIES = OTT_PLATFORMS.flatMap(platform =>
  OTT_GENRES.flatMap(genre =>
    OTT_TRENDS.map(trend => `${platform} ${trend}${genre}`)
  )
);
// 예: '넷플릭스 추천영화', '넷플릭스 인기영화', '넷플릭스 추천드라마', ...
//     '디즈니플러스 인기예능' 까지 총 24개

const SUPPLEMENT_QUERIES = [
  // [A] 웹툰·만화책 — 정보형(추천 리스트)은 최소화, 경험형 위주로 축소
  '웹툰 정주행 브이로그', '만화책 정주행 브이로그', '만화카페 웹툰 브이로그',

  // [A] OTT — 4개 플랫폼 × 영화/드라마/예능 × 추천/인기 (24개, 배경 정보용으로 유지하되 점수 가중치는 낮춤)
  ...OTT_QUERIES,

  // [A] OTT — 경험형 시청 브이로그 (실제 방문 욕구를 만드는 핵심 콘텐츠)
  '넷플릭스 보면서 힐링', '혼자 넷플릭스 정주행', '친구랑 영화보는날 브이로그',
  '커플 넷플릭스 데이트', '집콕 넷플릭스 브이로그',

  // [A] 보드게임 — 정보형보다 경험형(플레이 브이로그) 비중 확대
  '보드게임 카페 브이로그', '친구랑 보드게임 다녀옴', '보드게임 카페 데이트',

  // [B] 이색카페·룸카페 투어 ★ 경험형 핵심 — 공간·무드가 보이는 콘텐츠
  '이색카페 다녀온 후기', '룸카페 브이로그', '테마카페 데이트 브이로그',
  '만화카페 다녀옴', '아늑한 카페 브이로그', '힐링카페 브이로그',

  // [B] 혼자 놀기·혼놀 — 감성 무드가 드러나는 1인 콘텐츠
  '혼놀 브이로그', '혼자 힐링하는 시간', '혼자 카페에서 뒹굴뒹굴',

  // [B] 친구·모임 놀거리 — 웃음·리액션이 보이는 경험형
  '친구랑 놀러간 브이로그', '주말 놀거리 브이로그', '친구랑 갈만한 곳 후기',

  // [B] 데이트 코스 — 설렘·분위기가 보이는 경험형
  '데이트 코스 브이로그', '커플 데이트 브이로그', '커플 실내 데이트',

  // [B] 동네·대학가 핫플 — 실제 방문 리액션이 담긴 콘텐츠
  '동네 핫플 다녀옴', '숨은 핫플레이스 후기',
];
...
export async function fetchTrendData(apiKey, onProgress = () => {}) {
  ...
  const topKws = keywords.slice(0, 12);  // OTT 조합 쿼리 확장으로 쿼터 절약 위해 20→12 축소
      ...
      try {
        ...
        for (const it of (data.items || [])) {
          ...
          results.push({
            ...
            isExperience: isExperienceContent(title),  // 경험/무드형 콘텐츠 여부
            isBuzz:       isBuzzContent(title),         // 화제작형 콘텐츠 여부
            isInfoOnly:   isInfoOnlyContent(title),     // 순수 정보나열형 여부
    ...
    try {
      ...
      for (const it of (data.items || [])) {
        ...
        results.push({
          ...
          isExperience: isExperienceContent(title),
          isBuzz:       isBuzzContent(title),
          isInfoOnly:   isInfoOnlyContent(title),
        });
      }
    } catch (e) {
...
async function searchByKeyword(apiKey, kw) {
  ...
  return (data.items || [])
    ...
    .map(it => ({
      ...
      isExperience: isExperienceContent(it.snippet.title),
      isBuzz:       isBuzzContent(it.snippet.title),
      isInfoOnly:   isInfoOnlyContent(it.snippet.title),
    }));
}
...
// [A] 소비 관심사 — 방문객이 좋아하는 콘텐츠 주제 (만화책·웹툰 / OTT·예능 / 보드게임)
const INTEREST_KEYWORDS = [
  '웹툰', '만화', '만화책', '정주행', '완결', '연재',
  '넷플릭스', 'OTT', '드라마', '영화', '예능', '왓챠',
  '티빙', '웨이브', '디즈니플러스', '디즈니', 'HBO', '쿠팡플레이',
  '명작', '숨은명작', '정주행각',
  '보드게임', '보드게임카페',
];
...
const REPLICABLE_FORMAT_KEYWORDS = [
  ...
  '보드게임카페', '보드게임브이로그',
  // 만화책·OTT 시청 브이로그 (재현 가능 - 매장에서 보는 모습 그대로 촬영)
  '만화책브이로그', '웹툰브이로그', 'OTT브이로그', '넷플릭스브이로그', '정주행브이로그',
  // 핫플
  '핫플', '핫플레이스', '동네맛집', '대학가',
];
...
/**
 * ★ 핵심 로직 — 방문 욕구가 생기는 실제 심리 사슬 반영
 *
 * 예시:
 *   "이 만화책 재밌어 보이네 → 벌툰 가서 봐야지"
 *     (화제작 정보가 트리거 + "벌툰에 있다"는 연결이 완성)
 *   "넷플릭스 김부장 유행이네 → 보러 벌툰 가야지"
 *     (화제성 정보 + 벌툰에서 볼 수 있다는 연결)
 *   "이번 주 남자친구랑 데이트하러 벌툰 가고 싶다"
 *     (커플 데이트 경험 콘텐츠가 트리거)
 *
 * → 즉 "화제작이 뭔지 아는 것"(정보)과 "실제 즐기는 모습"(경험) 둘 다
 *   방문 욕구의 출발점이 될 수 있다. 진짜 핵심은 정보/경험 여부가 아니라
 *   "벌툰에서 즐길 수 있다"는 브릿지가 콘텐츠에 연결되는지 여부다.
 *   (브릿지는 analyzer.js의 액션 아이템·매장 가이드에서 필수로 삽입)
 *
 * 콘텐츠 3분류:
 *   [1] 화제작형(Buzz)   — 지금 뜨는 특정 작품·제목이 언급되는 콘텐츠
 *       예: "김부장 요즘 난리남", "이 웹툰 정주행 각"
 *       → "이거 뭐지, 나도 보고 싶다"는 욕구를 만듦
 *   [2] 경험형(Experience) — 실제 공간·관계·감정이 담긴 콘텐츠
 *       예: "커플이 데이트하며 노는 브이로그", "친구랑 놀러간 후기"
 *       → "나도 이렇게 놀고 싶다"는 욕구를 만듦
 *   [3] 순수 정보나열형(Info) — 위 두 특징이 전혀 없는 기계적 리스트
 *       예: "이번 주 개봉작 총정리" (특정 화제작 임팩트도, 경험도 없음)
 *       → 방문 욕구로 잘 이어지지 않음, 유일하게 낮은 가중치
 */
const EXPERIENCE_KEYWORDS = [
  // 방문·체험 인증
  '다녀왔', '다녀옴', '가봤', '가봤어요', '후기', '찐후기', '실제로',
  // 브이로그·일상 기록
  '브이로그', '데일리', '일상', '기록',
  // 감성·분위기
  '힐링', '아늑', '뒹굴', '무드', '감성', '분위기', '편안',
  // 관계·리액션
  '친구랑', '커플', '같이', '함께', '데이트', '놀러',
  // 리액션·감정 표현
  '웃참', '리액션', '완전 좋아함', '만족',
...
/**
 * 화제작형 판별 — 특정 작품·제목의 화제성을 다루는 콘텐츠
 * "OO이 요즘 난리다", "OO 정주행각", "OO 대박남" 같은 구체적 작품 언급형
 * → "이거 뭔지 궁금하다, 보고 싶다"는 욕구의 출발점이 되므로 긍정적으로 취급
 */
const BUZZ_KEYWORDS = [
  '난리', '대박', '화제', '레전드', '역대급', '실화', '충격',
  '요즘', '이번주', '신작', '최신', '떡상', '터짐',
  '정주행각', '정주행', '완결', '숨은명작', '꿀잼',
];

/**
 * 순수 정보나열형 — 화제성도 경험도 없는 기계적 리스트만 해당
 * (화제작 단어나 경험 단어가 하나라도 있으면 정보나열형에서 제외)
 */
const PURE_INFO_KEYWORDS = [
  'TOP', '순위', '총정리', '리스트', '모음', '베스트',
];

function isExperienceContent(title) {
  if (!title) return false;
  return EXPERIENCE_KEYWORDS.some(kw => title.includes(kw));
}

/** 화제작형 판별 — 구체적 작품·이슈가 화제성 있게 언급되는지 */
function isBuzzContent(title) {
  if (!title) return false;
  return BUZZ_KEYWORDS.some(kw => title.includes(kw));
}

/**
 * 순수 정보나열형 판별
 * 화제작 신호(BUZZ)나 경험 신호(EXPERIENCE)가 하나도 없고
 * 기계적 리스트 단어만 있는 경우에만 해당 (매우 좁은 범위)
 */
function isInfoOnlyContent(title) {
  if (!title) return false;
  const hasPureInfo = PURE_INFO_KEYWORDS.some(kw => title.includes(kw));
  const hasBuzz = isBuzzContent(title);
  const hasExperience = isExperienceContent(title);
  return hasPureInfo && !hasBuzz && !hasExperience;
}
